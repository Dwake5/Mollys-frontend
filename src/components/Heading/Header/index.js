...
class Header extends React.Component {
    ...
    componentDidMount = () => {
        ...
        if (!firstVisit) {
            ...
            if (date !== Number(localStorage.getItem('lastLogin'))) {
                ...
                localStorage.setItem('lastLogin', date)
            } else { // They have logged in before, but it was today so display the same quote.
                displaySameQuote()
            }
        }
    }
    ...
    displaySameQuote = () => {
        this.setState({ currentQuote: this.state.messages[localStorage.getItem('lastQuote')] })
    }
