...
import { ReviewDiv, Img, BottomText, MainText, Config, Button, ButtonDelete, ConfirmButton, Span } from './style'
...
class Review extends Component {
    ...
    handleName = name => {
        ...
        ? newName
        : newName += ' ' + this.capitalize(splitName[splitName.length-1].slice(0,1))
    }
    ...
    render() {
        const { data, service, review, name, showExtra, display, handleDisplaySwitch, handleDelete, chosenPicture, area } = this.props
        const { truncated, beenChanged, confirmDeleteButton } = this.state
        const { matchService, dontTruncate, doTruncate, shorten, handleName, openConfirm, capitalize } = this
        
        return (
            <ReviewDiv>
                <Img src={matchService(service, chosenPicture)}/>
                { !truncated && <MainText>{review}{beenChanged && <Span onClick={() => doTruncate()}>  See Less</Span>}</MainText> }
                { truncated && 
                ...
                {showExtra && (data.id > 0) &&
                    <Config>
                        ...
                        : <ConfirmButton onClick={() => handleDelete(data.id)}>Are you sure?</ConfirmButton>
                        }
                    </Config>
                }
            </ReviewDiv>
